...
export const EditParameter = ({
    ...
    >({
        ...
        defaultValues: {
            ...
            product: componentProps?.data?.product ?? "",
            code: componentProps?.data?.code ?? "",
            name: componentProps?.data?.name ?? "",
            kind: componentProps?.data?.kind ?? "",
            value_mode: componentProps?.data?.value_mode ?? undefined,
            percentage: componentProps?.data?.percentage ?? null,
            amount: componentProps?.data?.amount ?? null,
            calculation_base: componentProps?.data?.calculation_base ?? "",
            payee: componentProps?.data?.payee ?? "",
        },
    });

    const submitMutation = UseApiMutation<SubmitResponse, ParameterFormValues>({
        url: `finance/parameters/${componentProps?.data?.id}`,
    ...
    return (
        <div className="w-full min-w-150 max-w-150 p-6 space-y-6">
            <div className="border-b pb-3">
                <h2 className="text-xl font-semibold">Edit Parameter</h2>
